...
void FigTetris::figuraEncaixada(Fons& fons)
{
	...
	// Modifica els quadres corresponents a l'amplada de la figura actual a les posicions de la última línia del tauler del fons amb els colors de la figura actual

    for(int i = 0; i < m_amplada; i++)
    {
        fons.setTauler((m_posX/MIDA_Q) + i, MAX_FILA - 1 , m_color);
    }
}
